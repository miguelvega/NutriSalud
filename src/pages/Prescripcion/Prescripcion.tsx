...
import AdvertenciaPrescripcion from "./AdvertenciaPrescripcion";
...
const Prescripcion: React.FC = () => {
  ...
  const paciente = citas.find((cita) => cita.id === id)?.nombre || "Paciente no encontrado";

  const manejarCambioCita = (event: React.ChangeEvent<HTMLInputElement>) => {
    setHabilitarFecha(event.target.value === "si");
  };

  const validarEnteroPositivo = (valor: string, maxLength?: number): boolean => {
  ...
  const manejarCambioCalorias = (event: React.ChangeEvent<HTMLInputElement>) => {
    const valor = event.target.value;
    ...
    <div className="prescripcion-container">
      ...
      <div className="contenedor-principal">
        ...
        <div className="linea-datos-paciente">
          <p>
            ...
            <label>
              <input type="radio" name="sexo" value="masculino" defaultChecked /> Masculino
            </label>
          </p>
          <p>
            <strong>Edad:</strong>
      ...
      <div className="contenedor-plan">
        <div className="plan-dieta">
          ...
          <p>Alimentos recomendados: <input type="text" placeholder="_________" /></p>
          <p>Alimentos a evitar: <input type="text" placeholder="_________" /></p>
          <p>Observaciones:</p>
          <textarea className="static-textarea-large" maxLength={300} placeholder="Máximo 300 caracteres"></textarea>
        </div>
        ...
        <div className="plan-ejercicio">
          ...
          <p>
            ...
            <label>
              <input type="radio" name="tipo-ejercicio" value="flexibilidad" /> Flexibilidad
            </label>
            <label>
              <input type="radio" name="tipo-ejercicio" value="otro" defaultChecked /> Otro
            </label>
          </p>
          <p>Duración de las sesiones: <input type="text" placeholder="_________" /></p>
          <p>Frecuencia Semanal: <input type="text" placeholder="_________" /></p>
          <p>Ejercicios Específicos: <input type="text" placeholder="_________" /></p>
          <p>Observaciones:</p>
          <textarea className="static-textarea-large" maxLength={300} placeholder="Máximo 300 caracteres"></textarea>
        </div>
      </div>
